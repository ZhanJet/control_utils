...
path = os.getcwd()
data_w = csv_reader.read_csv(csvfile=path + '/plot/data_w.csv')
data_wo = csv_reader.read_csv(csvfile=path + '/plot/data_wo.csv')
...
# x_ee
axs[0,1].plot(data_w['Time'], data_w['x_des_w'], 'b-', label=r'$x^{ee}_{d}$', lw=1)
axs[0,1].plot(data_w['Time'], data_w['x_hat'], 'r-', label=r'$x^{ee}_{m}$', lw=1)
axs[0,1].legend(loc='upper left', mode="compact", ncol=1, fontsize=9, frameon=True)
axs[0,1].set_ylabel(r'$P_x$ (m)', fontsize=9)

axs[0,0].plot(data_wo['Time'], data_wo['x_des_w'], 'b-', label=r'$x^{ee}_{d}$', lw=1)
axs[0,0].plot(data_wo['Time'], data_wo['x_hat'],'r-', label=r'$x^{ee}_{m}$', lw=1)
axs[0,0].legend(loc='upper left', mode="compact", ncol=1, fontsize=9, frameon=True)
axs[0,0].set_ylabel(r'$P_x$ (m)', fontsize=9)

# V_ee & V_cmd
axs[1,1].plot(data_w['Time'], data_w['dx_hat'], 'r-', label=r'$V^{ee}_{m}$', lw=1)
axs[1,1].plot(data_w['Time'], data_w['wb_x_vel'], 'b-', label=r'$V^{ee}_{c}$', lw=1)
axs[1,1].plot(data_w['Time'], data_w['base_x_vel_flt'], 'g-', label=r'$V^{b}_{c}$', lw=1)
axs[1,1].plot(data_w['Time'], data_w['arm_x_vel_syn'], 'm-', label=r'$V^{a}_{c}$', lw=1)
...
axs[1,0].plot(data_wo['Time'], data_wo['dx_hat'],'r-', label=r'$V^{ee}_{m}$', lw=1)
axs[1,0].plot(data_wo['Time'], data_wo['wb_x_vel'], 'b-', label=r'$V^{ee}_{c}$', lw=1)
axs[1,0].plot(data_wo['Time'], data_wo['base_x_vel_flt'], 'g-', label=r'$V^{b}_{c}$', lw=1)
axs[1,0].plot(data_wo['Time'], data_wo['arm_x_vel_syn'],'m-', label=r'$V^{a}_{c}$', lw=1)
